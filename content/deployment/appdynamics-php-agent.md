...
4. Navigate to the directory where we have copied php agent rpm on the server and execute below command
   **APPD_CONF_CONTROLLER_HOST=controller
   APPD_CONF_CONTROLLER_PORT=443 APPD_CONF_APP=appname APPD_CONF_TIER=php
   APPD_CONF_NODE=node-name
   APPD_CONF_ACCOUNT_NAME=customer1
   APPD_CONF_ACCESS_KEY=acccesskey
